...
export const NewsHeadingsSectionOne = ({ data }: { data: any }) => {
    ...
    return <div className="container p-3">
        <div className="flex flex-col mx-auto gap-[1px] p-2">
            ...
            <div className="border-t border-t-black py-4 grid grid-cols-4 gap-5">
...
export const NewsHeadingsSectionThree = ({ data }: { data: any }) => {
    return <div className="container grid grid-cols-4 gap-5 p-3">

        {
            data.map((item: any, index: number) => {
                return <div className="flex flex-col gap-[1px]" key={index}>
                    <div>
                        <span className="px-3 py-1 rounded-full capitalize bg-black text-white">{item?.name}</span>
                    </div>
